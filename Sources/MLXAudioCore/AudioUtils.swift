...
///
/// Apple's `AVAudioConverter` (used by `resampleAudio`) has a gentler
/// near-Nyquist rolloff: downsampling 44.1k→16k leaves ~+14 dB of excess energy
/// in the 7–8 kHz band vs torchaudio. That extra brightness corrupts the
/// CAMPPlus speaker x-vector (cos ~0.64 to the reference), which conditions the
/// S3Gen flow decoder and makes regular Chatterbox output audibly brighter/edgier
/// than the PyTorch reference. Chatterbox conditioning uses this resampler so the
/// x-vector (and prompt tokens) match the reference.
